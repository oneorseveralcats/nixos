...
in
{
  options.programs.atool = {
    ...
    extraPackages = mkOption {
      ...
      default = [ ];
      example = lib.literalExpression "with pkgs; [ bzip2 cpio gnutar gzip lhasa lzop p7zip unrar-free unzip xz zip ]";
  ...
  in lib.mkIf cfg.enable {
    ...
    home.file.".atoolrc" = lib.mkIf (cfg.settings != { })  {
      source = pkgs.writeText ".atoolrc" (
        lib.generators.toKeyValue { mkKeyValue = lib.generators.mkKeyValueDefault { } " "; } cfg.settings
      );
    };
  };
}
