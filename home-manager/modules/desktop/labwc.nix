...
with lib;
let 
  cfg = config.myHome.desktop.labwc;
in
{
  options.myHome.desktop.labwc = {
    enable = lib.mkOption {
      description = "Enable and configure the labwc wayland compositor.";
      type = types.bool;
      default = false;
    };
  };
  ...
  config =  mkIf cfg.enable {
    ...
    xdg.configFile."labwc/environment".text = ''
      ...
      XKB_DEFAULT_LAYOUT=us(dvorak),us
      XKB_DEFAULT_OPTIONS=altwin:prtsc_rwin,caps:swapescape,lv3:ralt_switch_multikey,esperanto:dvorak,grp:shifts_toggle
